...
def animate(i, topography, dhdt, dx, K, area, m, n, uplift, dt, dtplot, plottime):
    ...
    erosionmax = max(dhdt * (-1000))    
    plot2.set_ydata(dhdt * (-1000))
    axis2.set_ylim([0.0, max(dhdt * (-1000)) * 1.1])
    erosiontext.set_y(max(dhdt * (-1000)) * 0.8)                      # Reposition time text
    erosiontext.set_text("Max erosion: {0:.1f} mm/a".format(erosionmax))   # Update time text
    #
    return plot1, plot2
...
simulation_time =100000.0 # Total simulation time [a]
...
# Format subplot 1
axis1 = plt.subplot(2,1,1)                  # Set axis1 as the first plot
...
# Format subplot 2
axis2 = plt.subplot(2,1,2)                  # Set axis2 as the second plot
axis2.set_xlim([0.0, max(xkm)])             # Set the x-axis limits for plot 2
axis2.set_ylim([0.0, max(dhdt * (-1000))* 1.1])    # Set the y-axis limits for plot 2
plot2, = plt.plot(xkm, dhdt * (-1000))          # Define plot2 as the second plot
plt.xlabel("Distance from drainage divide [km]")
plt.ylabel("Fluvial erosion rate [mm/a]")
# FIGURE 2 STUFF GOES HERE
erosionmax = max(dhdt)
erosiontext = plt.text(65., max(dhdt * (-1000))*0.8, "Max erosion: {0:.1f} mm/a".format(erosionmax))
